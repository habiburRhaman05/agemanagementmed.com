 ...
 *
 * Now animated: stats cascade in via StaggerGroup, quotes stagger with Reveal,
 * award logos fade-up as a StaggerGroup with grayscale→color on hover.
 */
  ...
  return (
    <Section background="alt" spacing="lg">
      <Container>
        ...
        <StaggerGroup stagger={0.1} className="mt-10 grid gap-10 border-b border-ink-900/10 pb-16 sm:grid-cols-3">
          {stats.map((stat) => (
        ...
        <div className="mt-16 grid gap-12 lg:grid-cols-2 lg:gap-20">
          {quotes.map((quote, index) => (
            <Reveal key={quote.id} delay={index * 90} side={index % 2 === 0 ? 'left' : 'right'}>
        ...
        {awards?.length ? (
          <StaggerGroup
            as="ul"
            stagger={0.07}
            className="mt-20 flex flex-wrap items-center justify-center gap-x-12 gap-y-8 border-t border-ink-900/10 pt-14"
          >
            {awards.map((award) => (
              <StaggerItem as="li" key={award.src}>
                <Image
                  src={award.src}
                  alt={award.alt}
                  width={110}
                  height={110}
                  className="h-16 w-auto opacity-70 grayscale transition-all duration-500 hover:opacity-100 hover:grayscale-0 hover:scale-105"
                />
              </StaggerItem>
            ))}
          </StaggerGroup>
